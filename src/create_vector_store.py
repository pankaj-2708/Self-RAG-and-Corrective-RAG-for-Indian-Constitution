...
embeddings = BedrockEmbeddings(
    model_id=model_name, region_name=wf_config["models"]["region"]
)
...
for key, value in zip(data.keys(), data.values()):
    ...
    documents.append(
        Document(metadata={"Article": article_no}, page_content=f"{key} \n: {value}")
    )
...
for key, value in data.items():
    ...
    documents.append(
        Document(metadata={"Section": section_no}, page_content=f"{key} \n{value}")
    )
